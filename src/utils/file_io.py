...
from exception.custom_exception import CatalystAIException
...
def generate_session_id(prefix: str = "session") -> str:
    ...
    return f"{prefix}_{datetime.now(cst).strftime('%Y%m%d_%H%M%S')}_{uuid.uuid4().hex[:8]}"
...
def save_uploaded_files(uploaded_files: Iterable, target_dir: Path) -> List[Path]:
    ...
    try:
        ...
        raise CatalystAIException("Failed to save uploaded files", e) from e